...
#     def __init__(self, parent=None):
#         super().__init__(parent)
#         self.setWindowTitle("Select or Create Model")
#         self.layout = QVBoxLayout()
...
#             self.model_name, ok = QInputDialog.getText(self, "Input Dialog", "Enter model name:")
#             if ok and self.model_name and not self.model_name.isspace():
#                 os.makedirs(os.path.join(model_dir, self.model_name))
#                 self.close()
#             else:
#                 QMessageBox.warning(self, "Invalid Input", "Model name cannot be empty or spaces.")
#         else:
#             if not os.listdir(model_dir):  # directory is empty
#                 self.model_name, ok = QInputDialog.getText(self, "Input Dialog", "Enter model name:")
#                 if ok and self.model_name and not self.model_name.isspace():
#                     os.makedirs(os.path.join(model_dir, self.model_name))
#                     self.close()
...
#                 self.model_name, ok = QInputDialog.getItem(self, "Select Dialog", "Select model:", os.listdir(model_dir), 0, False)
#                 if ok and self.model_name:
...
#
#
#         self.available_colors = ['cyan', 'red', 'green', 'pink', 'yellow', 'blue', 'brown', 'purple', 'black',
#                                  'white']
#
...
#
#
#         self.model_name = self.model_dialog.model_name
#
#
#
...
#             if not os.path.exists(self.csv_label_file):
#                 df = {"FileName": [], "Tool bounding box": []}
#                 df = pd.DataFrame(df)
#                 df["FileName"] = self.image_files
#                 df["Tool bounding box"] = '[]'
...
#                     pass
#
#     def predict_next_set(self, state):
#         msg_box = QMessageBox()
...
#                 print("less")
#                 indexes = np.linspace(0, len(self.image_files) - 1, self.frames_to_annotate_spinbox).astype(int)
...
class Annotator(QMainWindow):
    ...
    def copyAndPasteBoundingBoxes(self):
        if self.current_index > 0:
            ...
            if os.path.exists(previous_label_path):
                ...
                # Filter lines that start with "0" or "1"
                filtered_lines = [line for line in previous_bounding_boxes if line.startswith(("0", "1"))]

                if previous_bounding_boxes != []:
                    # Check if the current label file exists and is not empty
                    if os.path.exists(current_label_path) and os.path.getsize(current_label_path) > 0:
                        # Append the filtered lines to the current label file
                        with open(current_label_path, "a") as current_file:
                            current_file.writelines(filtered_lines)
                    else:
                        # Write the filtered lines to the current label file
                        with open(current_label_path, "w") as current_file:
                            current_file.writelines(previous_bounding_boxes)
    ...
    def AutoAnnotateProcess(self):
        ...
        self.simulator = Simulator(self.image_folder)
